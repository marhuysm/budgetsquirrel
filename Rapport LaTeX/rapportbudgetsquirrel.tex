...
Une fois qu'il a effectué toutes les actions qu'il voulait, il a la possibilité de se déconnecté, et est alors redirigé vers l'écran d'accueil.
...
Même si ce n'est pas le point central de notre projet, une situation d'utilisateur au profil administratif, qui peut gérer la base de données, est également prévu.
...
\item Dans la table \verb\tf_carte\, \verb\transaction_financière.num_tf\ et \verb\carte.numero_carte\ référencent obligatoirement le même \verb\niss_util\ : un utilisateur ne peut qu'utiliser les cartes qui lui sont liées pour les transaction qui lui sont liées;
\item \verb\transaction_financiere.num_tf\ doit se retrouver une et une seule fois soit dans la table \verb\tf_carte\, soit dans la table \verb\tf_virement\, soit dans la table \verb\tf_cash\; \textbf{Côté DB, on ne vérifiait pas, j'ai donc ajouté 3 trigger qui le font (1 pour chaque table)}
\item \textbf{Autres?}
\end{itemize}
...
Il en est de même pour la table \verb\transaction_financière\, à laquelle nous avons ajouté une référence à la fois au \verb\budget_mensuel.budget_id\, et à \verb\utilisateur.niss\.
Toujours en suivant la même logique, l'entité faible \verb\carte\ contient son attribut d'association identifiante, lié au NISS de l'utilisateur, et la clé de la table est formée de la combinaison NISS - numéro de carte; la table \verb\transaction_financiere\ contient l'attribut qui référence \verb\categorie_tf.nom_tf\; et \verb\tf_carte\ contient les références à \verb\transaction_financière.num_tf\ et à \verb\carte.numero_carte\. 

\textbf{A CONFIRMER : } Nous n'avons pas dû traduire l'association n-aire entre \verb\transaction_financiere\, \verb\budget_mensuel\ et \verb\utilisateur\, tout simplement car elle n'est pas nécessaire : la table \verb\transaction_financiere\ contient déjà toutes les informations de l'association, à savoir l'association \verb\num_tf\ - \verb\budget_id\ - \verb\niss\. Une table séparée représentant le lien entre les trois tables aurait été nécessaire si, par exemple, des données étaient directement liée à l'association qui les lie.
...
\textbf{?? qu'est ce qu'il faudrait mettre ici?}
...
\textbf{Tu avais parlé de structure HTML ouverte, mais j'ai corrigé en parlant de CSS, corrige moi si je me trompe though}
...
\textbf{Les points forts et faiblesses d’application, le temps nécessaire pour développer le concept avec deux personnes, les sources consultées pour la partie pratique (i.e. les slides, ou techniques vues au TP, ainsi que les conseils d’assistante, mais aussi w3schools ou bien stackoverflow).}
\textbf{Finalement quelques mots sur l'expérience du projet.
}
\textbf{Améliorations : 
- vérification du niss par rapport à la date de naissance de l'utilisateur - finalement, le bilan dans la table budget mensuel n'est pas obligatoire, pour éviter des informations redondantes - choix de la matérialisation pour la traduction de l'héritage : remise en question - ajout d'un enregistrement de connexions pour chaque utilisateur ainsi qu'un système de log in et mot de passe - l'ajout d'une interface admin du côté de l'application}
